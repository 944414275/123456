...
namespace TX.Framework.WindowUI.Forms
{
    ...
    public partial class BaseForm
    {
        ...
        //成功按钮
        protected DialogResult Succeed(string captionText, string message)
        {
            return TXMessageBoxExtensions.Succeed(captionText, message);
        }

        protected DialogResult Succeed(string message)
        {
            Random ran = new Random(DateTime.Now.Second);
            return this.Succeed(this.iErrorCaptions[ran.Next(0, this.iErrorCaptions.Length)], message);
        }
